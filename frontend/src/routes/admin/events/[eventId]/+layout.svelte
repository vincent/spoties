<script lang="ts">
  ...
  const activeClass =
    "rounded-none md:border-b-2 border-primary-600 text-primary-600";
...
<div class="mx-auto w-full max-w-(--breakpoint-2xl)">
  <Navbar class="mb-5" navContainerClass="flex justify-center">
    <NavUl
      {activeUrl}
      ulClass="flex flex-col p-4 pb-0 mt-4 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:text-sm md:font-medium"
    >
      {#if data.eventId !== "create"}
        <NavLi {activeClass} href={`/admin/events/${data.eventId}/`}
          >{$t("menu.form")}</NavLi
        >
