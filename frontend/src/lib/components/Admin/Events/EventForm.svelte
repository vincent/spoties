...
<form
  class="flex flex-col items-start justify-center xl:flex-row"
  onsubmit={submitForm}
>
  <div class="w-full xl:w-3/4">
    ...
    {#if $store.id}
      <div class="mb-6 flex justify-between">
        <div>
          <Label class="mb-2 hidden text-2xl md:block"
            >{$t("event.form.public_link")}</Label
          >
          <A target="_blank" href={publicLink}
            ><Link2Icon class="mr-2" /> {publicLink}</A
          >
        </div>
        <div>
      ...
      <Accordion>
        ...
        >
          ...
          <FieldErrors
            ...
            validationErrors={validation?.error?.fieldErrors?.description}
          />
        </AccordionItem>
        ...
        {#if showHelp && help.questions}
          <EventFormHelp
            ...
            disabled={!$store.questions?.length ||
              !!validation?.error?.fieldErrors?.questions}
            class="mb-6"
        ...
        {#if showHelp && help.locations}
          <EventFormHelp
            {next}
            disabled={!$store.locations?.length ||
              !!validation?.error?.fieldErrors?.locations}
            class="mb-6"
  ...
  <div class="mt-3 w-full xl:sticky xl:top-18 xl:mt-0 xl:ml-3 xl:w-1/4 xl:ps-4">
