<script lang="ts">
  ...
  let openRow = $state(-1);
  let slotSpan = $derived(
    event.locations.reduce((acc, l) => acc + l.slots.length, 0)
  );
  ...
  const toggleBookingSlot = (slot) => {
    ...
    const without = toMap(
      Object.keys(manualBookings.slots).filter((s) => s !== slot.id)
    );
    manualBookings = {
      ...manualBookings,
      pristine: false,
  ...
  const saveManualBooking = (body) =>
    Promise.resolve()
      .then((_) => (manualBookingLoading = true))
      .then((_) =>
        client
          .send(`/api/admin/events/${event.id}/saveManualBooking`, {
            method: "post",
            body: { ...manualBookings, ...body },
          })
          .then(async () => {
              ...
              {#if openRow === i}
                <CircleCheckIcon
                  class="hover:opacity-100
                    ...
                    {response.bookings.includes(s.id)
                    ? 'text-primary-700'
                    : 'text-grey-300 dark:text-primary-100'}
                    {manualBookings.slots[s.id]
                    ? ''
      ...
      {#if openRow === i}
        <TableBodyRow>
          <TableBodyCell colspan={2 + slotSpan} class="p-6">
            <div
              class="grid pt-8 text-left md:grid-cols-2 md:gap-16 dark:border-gray-700"
            ...
            >
              ...
              <div class="text-end">
                <Button
                  ...
                  >{$t("event.results.save_slots_selection")}
                  <ChevronDownOutline
