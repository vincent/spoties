<script lang="ts">
  ...
  let {
    removeQuestion,
    questionIndex,
    value = $bindable<QuestionsRecord>(),
  } = $props();
  ...
  function updateAnswerType(answer_type: QuestionType) {
    ...
    if (
      ["private_name", "private_age", "private_address"].includes(answer_type)
    )
      value = {
        ...value,
        label: $t(`event.form.question_type_${answer_type}` as any),
      };
    ...
    if (answer_type === "select_one" && !value.properties?.choices)
      value = {
        ...value,
        properties: { choices: [{ name: $t("data.option_i", { i: 1 }) }] },
      };
  ...
  function removeChoiceOption(choiceIndex: number) {
    value = {
      ...
      properties: {
        ...(value.properties || {}),
        choices: (value.properties?.choices || []).filter(
          (c, i) => i !== choiceIndex
        ),
      },
    };
  ...
  function addChoiceOption() {
    value = {
      ...
      properties: {
        ...(value.properties || {}),
        choices: (value.properties?.choices || []).concat({
          name: $t("data.option_i", {
            i: (value.properties?.choices?.length || 0) + 1,
          }),
        }),
      },
    };
...
<Card size="xl" class="mt-2 p-4">
    ...
    <div class="mr-auto flex w-3/6 flex-col">
      ...
      >
        <h5 class="mb-2 text-xl tracking-tight text-gray-900 dark:text-white">
          {@html $AdminEventStore.questions[questionIndex].label ||
            `<em>${$t("event.form.question_title")}</em>`}
        </h5>
      </EditInPlace>
  ...
  {:else if value.answer_type === "rich_text" && value.properties?.rich_placeholder !== undefined}
    <FloatingLabelInput
      type="text"
      bind:value={value.properties.rich_placeholder}
    >
      {$t("event.form.placeholder")}
    </FloatingLabelInput>
  ...
  {:else if value.answer_type === "checkboxes" && value.properties?.choices !== undefined}
    {#each value.properties.choices as choice, ci}
      <div class="my-1 flex items-center justify-between">
        ...
        <Input size="sm" bind:value={value.properties.choices[ci].name} />
      </div>
    {/each}
    <button
      type="button"
      class="mt-3 inline-flex items-center text-sm font-medium text-gray-500 transition-colors duration-200 hover:text-gray-900 hover:underline dark:text-gray-400 dark:hover:text-white"
      onclick={addChoiceOption}
    >
      <PlusOutline class="mr-2" />
      {$t("event.form.add_choice")}
    </button>
  {:else if value.answer_type === "select_one" && value.properties?.choices?.length}
    {#each value.properties.choices as choice, ci}
      <div class="align-content-center my-1 flex items-center justify-between">
        <DeleteButton
          btnClass="me-2"
          skip
          confirm={() => removeChoiceOption(ci)}
        />
        <Input size="sm" bind:value={value.properties.choices[ci].name} />
      </div>
    {/each}
  ...
  {:else if value.answer_type === "select_many" && value.properties?.choices?.length}
    {#each value.properties.choices as choice, ci}
      <div class="align-content-center my-1 flex items-center justify-between">
        <input type="radio" disabled class="mr-2 border-gray-500" />
        <Input size="sm" bind:value={value.properties.choices[ci].name} />
      </div>
    {/each}
    <button
  ...
  {#if value.answer_type !== "just_text"}
    <div class="mt-4 flex justify-end">
      <Toggle bind:checked={value.required} class="italic dark:text-gray-500"
        >{$t("data.required")}</Toggle
      >
    </div>
  {/if}
</Card>
