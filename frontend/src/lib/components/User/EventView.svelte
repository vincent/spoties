<script lang="ts">
  ...
  const somePreviousAnswers =
    userData?.bookings?.id ||
    Object.values(userData?.questions_answers || {}).some((a: any) => a.value);
  ...
  let qValidation = $derived<Record<string, ZodIssue>>(
    (validation?.error?.fieldErrors?.questions_answers || []).reduce(
      (acc, qe) => ({
        ...acc,
        [qe.path[1]]: qe,
      }),
...
<NavMini>
  <h1 class="mt-6 mb-10 block text-4xl text-gray-800 dark:text-gray-100">
    {record.title}
  </h1>
</NavMini>
    ...
    {#each record.questions as q}
      {#if !q.deleted}
        ...
        <div class="space-y-4">
          <Card
            size="xl"
            class="mt-2 p-4 qtype-{q.answer_type} {considerAnswered(
              $store.questions_answers[q.id]?.value
            ) || q.answer_type === 'just_text'
          ...
          >
            ...
            {:else if q.answer_type === "simple_text"}
              <Input
                ...
                bind:value={$store.questions_answers[q.id].value}
              />
            ...
            {:else if q.answer_type === "rich_text" && !disabled}
              <RichText
                class="rounded-lg border border-gray-300 dark:border-gray-500"
                size={8}
                bind:value={$store.questions_answers[q.id].value}
              />
            ...
            {:else if q.answer_type === "yes_no"}
              <Toggle
                {disabled}
                checked={$store.questions_answers[q.id].value || false}
                onchange={(e) =>
                  ($store.questions_answers[q.id].value = (
                    e?.target as any
                  )?.checked)}
            ...
            {:else if q.answer_type === "checkboxes" && props.choices !== undefined}
              {#each q.properties?.choices as choice, ci}
                <Checkbox
                  {disabled}
                  value={$store.questions_answers[q.id].value}
                  group={q.properties}
                  onchange={(e) =>
                    (q.properties = (q.properties || []).concat(
            ...
            {:else if q.answer_type === "select_one" && props.choices?.length}
              {#each q.properties?.choices as choice, ci}
                <Radio
                  {disabled}
                  class="my-1"
                  bind:group={$store.questions_answers[q.id].value}
                  value={choice.name}>{choice.name}</Radio
                >
              {/each}
            {:else if q.answer_type === "select_many"}
              <MultiSelect
                {disabled}
                size="lg"
                items={multiSelectChoices(q.properties?.choices)}
                bind:value={$store.questions_answers[q.id].value}
              />
            {/if}

            <FieldErrors
              validationErrors={qValidation[q.id] ? [qValidation[q.id]] : null}
            />
          </Card>
        </div>
      {/if}
      ...
      {#each record.locations as l, i}
        {#if !l.deleted}
          <div class="mb-4 space-y-4">
            <Card
              size="xl"
              class="p-4 {l.slots.find((s) => $store.bookings.slots[s.id])
                ? 'border-primary-600 dark:border-secondary-800 border-2'
                : ''}"
            >
              <div class="mb-3 flex">
                ...
                {#if !s.deleted}
                  <div class="my-3">
                    <Checkbox
                      class="my-1 flex"
                      disabled={disabled ||
                        (s.limit > 0 && s.available_seats <= 0)}
                      value={s.id}
                      checked={$store.bookings.slots[s.id]}
                      onchange={(e) =>
                        ($store.bookings.slots[s.id] = (
                          e?.target as any
                        )?.checked)}
                    >
                      <div class="mr-auto">
    ...
    <div class="mb-8 flex justify-end-safe">
      <Button
        ...
        {#if $store.loading}<Spinner
          ...
          />{:else}<ArrowRightOutline class="ml-2" />{/if}</Button
      >
    </div>
  {/if}
</form>
